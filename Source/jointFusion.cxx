...
int jointFusion( itk::ants::CommandLineParser *parser )
{
  ...
  if( verbose )
    {
    std::cout << std::endl << "Running jointFusion for "
  ...
  if( searchRadiusOption && searchRadiusOption->GetNumberOfFunctions() )
    ...
    if( itksys::SystemTools::FileExists( searchRadiusString.c_str() ) )
      ...
      typedef itk::ImageFileReader<RadiusImageType> SearchImageReaderType;
      typename SearchImageReaderType::Pointer reader = SearchImageReaderType::New();
      reader->SetFileName( searchRadiusString.c_str() );

      searchRadiusImage = reader->GetOutput();
      searchRadiusImage->Update();
      searchRadiusImage->DisconnectPipeline();
      }
    else
      {
...
//     }
  ...
  if( targetImageOption && targetImageOption->GetNumberOfFunctions() )
    ...
    if( targetImageOption->GetFunction( 0 )->GetNumberOfParameters() == 0 )
      ...
      typedef itk::ImageFileReader<ImageType> TargetImageReaderType;
      typename TargetImageReaderType::Pointer reader = TargetImageReaderType::New();
      reader->SetFileName( targetFile.c_str() );

      targetImage = reader->GetOutput();
      targetImage->Update();
      targetImage->DisconnectPipeline();
  ...
  for( unsigned int m = 0; m < numberOfAtlases; m++ )
    ...
    if( atlasImageOption->GetFunction( m )->GetNumberOfParameters() == 0 )
      ...
      typedef itk::ImageFileReader<ImageType> AtlasImageReaderType;
      typename AtlasImageReaderType::Pointer reader = AtlasImageReaderType::New();
      reader->SetFileName( atlasFile.c_str() );

      atlasImage = reader->GetOutput();
      atlasImage->Update();
      atlasImage->DisconnectPipeline();
    ...
    if( numberOfAtlasSegmentations > 0 )
      ...
      typedef itk::ImageFileReader<LabelImageType> AtlasSegmentationReaderType;
      typename AtlasSegmentationReaderType::Pointer reader = AtlasSegmentationReaderType::New();
      reader->SetFileName( atlasSegmentationFile.c_str() );

      atlasSegmentation = reader->GetOutput();
      atlasSegmentation->Update();
      atlasSegmentation->DisconnectPipeline();
      }
  ...
  if( exclusionImageOption && exclusionImageOption->GetNumberOfFunctions() )
    ...
    for( unsigned int n = 0; n < exclusionImageOption->GetNumberOfFunctions(); n++ )
      ...
      typedef itk::ImageFileReader<LabelImageType> ExclusionImageReaderType;
      typename ExclusionImageReaderType::Pointer reader = ExclusionImageReaderType::New();
      reader->SetFileName( exclusionFile.c_str() );

      exclusionImage = reader->GetOutput();
      exclusionImage->Update();
      exclusionImage->DisconnectPipeline();
  ...
  if( maskImageOption && maskImageOption->GetNumberOfFunctions() )
    ...
    typename MaskImageReaderType::Pointer reader = MaskImageReaderType::New();
  ...
  if( outputOption && outputOption->GetNumberOfFunctions() )
    ...
    if( !labelFusionName.empty() )
      ...
      writer->SetFileName( labelFusionName.c_str() );
    ...
    if( !intensityFusionName.empty() )
      ...
      for( unsigned int i = 0; i < imageNames.size(); i++ )
        ...
        writer->SetInput( jointIntensityFusionImage );
        writer->Update();
        }
      }
    if( !labelPosteriorName.empty() && fusionFilter->GetRetainLabelPosteriorProbabilityImages() )
      ...
      for( labelIt = labelSet.begin(); labelIt != labelSet.end(); ++labelIt )
        ...
        typedef itk::ImageFileWriter<typename FusionFilterType::ProbabilityImageType> WriterType;
        typename WriterType::Pointer writer = WriterType::New();
        writer->SetFileName( buffer );
        writer->SetInput( fusionFilter->GetLabelPosteriorProbabilityImage( *labelIt ) );
        writer->Update();
        }
      }
    if( !atlasVotingName.empty() && fusionFilter->GetRetainAtlasVotingWeightImages() )
      ...
      for( unsigned int i = 0; i < imageNames.size(); i++ )
        ...
        typename WriterType::Pointer writer = WriterType::New();
        writer->SetFileName( imageNames[i].c_str() );
        writer->SetInput( fusionFilter->GetAtlasVotingWeightImage( i ) );
        writer->Update();
        }
      }
    }
...
int main( int argc, char **argv )
{
  ...
  std::string commandDescription =
    std::string( "jointFusion is an image fusion algorithm developed by Hongzhi Wang and " )
  ...
  switch( dimension )
    ...
    case 2:
      {
      return jointFusion<2>( parser );
      }
      break;
    case 3:
      {
      return jointFusion<3>( parser );
      }
      break;
    case 4:
      {
      return jointFusion<4>( parser );
      }
      break;
    default:
